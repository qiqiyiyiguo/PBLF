...
// 添加新用户时检查游客数量
void add_user(name_str, number_str, password_str) {
    ...
    if (user_count < MAX_USERS) {
        ...
        if (strcmp(is_guest, "yes") == 0 && guest_count >= MAX_GUESTS) {
...
int query_appointmentcount(const char* date, const char* campus) {
    ...
    while (fgets(line, sizeof(line), file)) {   //读取每一行的信息
        ...
        if (strcmp(date, file_date) == 0 && strcmp(campus, file_campus) == 0) {  //比对信息相同，预约数量加1
            count++;   
        }
    }
    fclose(file)//关闭文件
...
pthread_create(&cleanup_thread, NULL, void session_cleanup_thread, NULL);
