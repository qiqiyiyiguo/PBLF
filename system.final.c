...
int login_user(const char* name, const char* password, visitor* user, int userNumber); //登录用户
...
static int callback_websocket(struct lws* wsi, enum lws_callback_reasons reason,void* user, void* in, size_t len) {
    ...
    case LWS_CALLBACK_RECEIVE: {
        ...
        if (strcmp(task, "register") == 0) {
            ...
            // 获取预约信息
            const char* appointment_time = cJSON_GetObjectItem(root, "appointment_time")->valuestring;
            const char* details = cJSON_GetObjectItem(root, "details")->valuestring;

            Appointment appointment;
            strcpy(appointment.userName, name);
            strcpy(appointment.appointment_time, appointment_time);
