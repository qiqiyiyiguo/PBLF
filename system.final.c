...
void broadcast(struct session_data *data, unsigned char *message, size_t len);
...
// 用于跟踪已登录用户
struct lws* clients[100]; // 存储客户端连接
int client_count = 0;      // 客户端连接数
...
//主函数
int main() {
    memset(&info, 0, sizeof(info));     //û�ҵ��ú��� 
    info.port = 8080;  // 服务器监听的端口
    info.protocols = protocols;
    info.gid = -1;
    info.uid = -1;

    // 创建 WebSocket 上下文
    context = lws_create_context(&info);  //error 
    if (context == NULL) {
    ...
    if (check_password()) {
        ...
        init_list(); // 初始化链表
        load_users(user, userNumber); // 加载用户数据到内存
        while (1) {
            print_menu(); // 打印菜单
            printf("请选择系统功能项：\n");
            scanf("%c", &choice); // 输入选择
...
printf("Password strength: %s\n", strength);
    ...
    {   
        ...
        timer.it_interval.tv_usec = 0;
              ...
              signal(SIGALRM, timer_handler);//timer_handlerû������ 
...
// 读取所有用户数据到内存中
int load_users(visitor* user, int userNumber) {
    ...
    userNumber = 0;
    while (fscanf(file, "%49s,%49s,%49s\n", user[userNumber].name, user[userNumber].id, user[userNumber].password) == 3) {  //输入名字，身份证号，密码
        userNumber++;
    }
...
// 检查用户是否已存在
int user_exists(const char* name, visitor* user, int userNumber) {
...
int register_user(const char* name, const char* id, const char* password, visitor* user, int userNumber) {  //模拟用户注册
    if (user_exists(name, user, userNumber)) {
        return -1;  // 用户已存在
    }
}
 //登录用户
int login_user(const char* name, const char password[], visitor* user, int userNumber) {
    for (int i = 0; i < userNumber; i++) {
...
static int callback_websocket(struct lws* wsi, enum lws_callback_reasons reason,void* user, void* in, size_t len) {
    ...
    // 加载用户数据
    int userNumber = 0;
    if (load_users(user, userNumber) != 0) {
        snprintf(message, sizeof(message), "Failed to load user data");
    ...
    case LWS_CALLBACK_RECEIVE: {
        ...
        const char* task = cJSON_GetObjectItem(root, "task")->valuestring;
        ...
        if (strcmp(task, "register") == 0) {
            int result = register_user(name, id, password, user, userNumber);
            if (result == 0) {
                snprintf(message, sizeof(message), "success");
            }
            else if (result == -1) {
                snprintf(message, sizeof(message), "User already exists");
            }
            else {
                snprintf(message, sizeof(message), "Registration failed");
            }
        }
        else if (strcmp(task, "login") == 0) {
            int result = login_user(name, password, user, userNumber);
            if (result == 0) {
                // 登录成功，记录客户端
                clients[client_count++] = wsi; // 将 WebSocket 客户端保存到列表
                snprintf(message, sizeof(message), "success");
            }
            else {
                snprintf(message, sizeof(message), "Invalid credentials");
            }
        }
        else if (strcmp(task, "appointment") == 0) {
            // 获取预约信息
            const int startHour = cJSON_GetObjectItem(root, "startHour")->valueint;
            const int endHour   = cJSON_GetObjectItem(root, "endHour")->valueint;
            const int month     = cJSON_GetObjectItem(root, "month")->valueint;
            const int day       = cJSON_GetObjectItem(root, "day")->valueint;
            const int year      = cJSON_GetObjectItem(root, "year")->valueint;
            const long long int phonenumber=cJSON_GetObjectItem(root,"phonenumber")->valueint;
            const char campus_name=cJSON_GetObjectItem(root,"campus_name")->valueint;
            
            Appointment appointment;
            strcpy(appointment.userName, name);
            appointment.startHour=startHour;
            appointment.endHour=endHour;
            appointment.month=month;
            appointment.day=day;
            appointment.year;
            appointment.phonenumber=phonenumber;
            strcpy(appointment.campus_name, campus_name);
            // 保存预约信息到文件
            int result = save_appointment(&appointment);
            if (result == 0) {
                snprintf(message, sizeof(message), "Appointment saved successfully");
            }
            else {
                snprintf(message, sizeof(message), "Failed to save appointment");
            }
        }
        else {
            snprintf(message, sizeof(message), "Unknown task");
        }// 发送响应
        lws_write(wsi, (unsigned char*)message, strlen(message), LWS_WRITE_TEXT);
        cJSON_Delete(root);
        break;
...
void insert_appointment() { // 预约
    ...
    // 打印该用户所有的预约信息
    print_all_appointments_for_user(p->appointmentInfo.userName);
    for (i = 0; i < userNumber; i++) {
        if (strcmp(user[i].id, ID) == 0) {
...
printf("\n");
    ...
    printf("The booking is successful!\n");
    p->appointmentInfo.next = head;
    head = p->appointmentInfo.next;
    res[reserver++] = p->appointmentInfo;
}

void print_all_appointments_for_user(char* username) {
    int found = 0;
    for (int i = 0; i < reserver; i++) {
        if (strcmp(res[i].userName, username) == 0) {
            printf("Reservation: starthour: %d endhour: %d\nDate: day: %d, month: %d, year: %d\nphonenumber: %lld\n",
